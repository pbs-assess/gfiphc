---
title: "Check the 2022 data and incorporate it into gfiphc"
...
In 2022 (as for 2020 and 2021) only the first 20 hooks were evaluated, so those data are not easily imported into
...
full survey so need to do the maps here to undersatnd.
...
For 2021 and 2022 we have all as Standard Grid, which gets corrected (some stations are
...
folder as `set-and-halibut-data-2022.xlsx`. Open in Excel and Export as .csv, `set-and-halibut-data-2022.csv`,
...
`non-halibut-data-2022.xlsx` and export as .csv in Excel,
`non-halibut-data-2022.csv`. Importantly, this file
...
not sufficient. For 2022 we so far have this:
```{r map2022}
...
Now to figure out standard/non-standard stations. Plotting four years, with crosses showing 'non-standard'. (2022 is coloured
different (empty it seems) since no hooks with bait data yet, but the important bit is the crosses).
```{r maps}
...
Plot stations not standard in 2018 but standard in 2022, and vice versa, using
...
stations are non-standard in 2018 or 2022. Also (for 2021 and then 2022) adding all stations,
...
the non-standard 2018 stations. Black dots (2022 stations) with no green squares
off WCVI clearly shows the reduced coverage there (presumably that should say
empty green squares with no black dots - similar for 2021 and 2022).
...
#  recently consistent
# For 2022 checking before saving any data into gfiphc.
previous_stations <- yelloweye_rockfish$set_counts$station %>%
  unique()
...
as a blue rectangle shows ... (see saved .pdf).
...
and call both new 2022 stations non-standard (like we did for the six new 2021 stations):
```{r corrected}
...
dplyr::filter(hooks_with_bait$set_counts,
  ...
  unique()
```
Note 2022 won't show up here until .rda objects are resaved in package, at the
...
```
201: Presumably Sun Sea Star and Sunflower Sea Star are the same. Will mention this later on.

**Work out what to do with all those new ones.
...
**NOT EVALUATING YET UNTIL GONE THROUGH pdf
