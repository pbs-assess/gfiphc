...
This is for 2020 (keep but comment out for future years):
```{r understand}
...
HAN. Do this and simplify down to what's needed and rename, based on `iphc2013data.Rnw` (need
...
Doing this having already saved .rda files for 2020, crosses show 'non-standard'
```{r maps}
...
sets_2020 <- filter(hooks_with_bait$set_counts,
                    year == 2020)
...
each years' lats and lons (to verify that they all still agree -- station
...
Check if the one standard in 2018 but not in 2020 (and doesn't appear in 2019)
...
Great. Was only those two years, so we should define it as non-standard.

TODO - try and do the above so it still works with the .rda files updated -
i.e. use the raw data that's imported here.
