...
Take the halibut counts for all the hooks and create `N_it20 = E_it20 / E_it * N_it`. Not
   strictly the first 20 hooks, but is a rescaling. But will not guarantee integer values.

```{r sethalibut}
setData2020_and_halibut <- setData2020 %>%
  dplyr::left_join(    by = "station")   # TODO probably keep halibut counts in
                                        # sets_simp and sets_simp_std earlier, since accounts for
                                        # the doubly fished station
```
...
```{r countsmaybe}
counts_maybe_halibut <- countData2020_no_halibut %>%
  dplyr::group_by(station) %>%
  dplyr::summarise(non_halibut = sum(specCount)) %>%
  dplyr::ungroup()
counts_maybe_halibut
```
