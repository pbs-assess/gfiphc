...
shows that we should call the five northern ones non-standard also, to exclude
from the standard Series A-F analyses. Maybe zoom in on the one off the
northwest of Vancouer Island:
...
So that precise location was clearly avoided in the past (exactly spaced grid
implies it would be on a small island called Lanz Island), but it's not exactly
moving into new territory. It is station 2257 (see above), with a depth of only
40~m (I think it's metres), which is not an outlier. For example, for 2013
(depth data for all years is not in gfiphc I don't think):
```{r depth2013}
sort(setData2013$avgDepth)
```
So let's call it a standard station, as it's in a survey area and not
ridiculously shallow.

So - retain the 2018 definitions of standard stations (as we did for 2020),
and call any new 2021 stations non-standard, except for 2257:
```{r corrected}
...
sets_simp_std_corrected$standard[sets_simp_std_corrected$station == "2257"] <- "Y"
summary(as.factor(sets_simp_std_corrected$standard))
