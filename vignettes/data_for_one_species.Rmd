...
library(gfiphc)
library(dplyr)    # Need to print tibble properly
```
...
setData2020      # Station details for 2020 survey
...
**NOTE** As noted in Issue #14, the standard refinement has not yet been done
for the Series A, B, C, D etc. calculation code that was used for the synopsis report, but the latest
Yelloweye Rockfish assessment did properly account for this.

COPYING FROM RESTRICTED AREA:
...
This section does those calculations in detail to aid understanding (there is a
also a wrapper function to automatically do all the steps).

Calculate Series A, B, C, and D, and Series AB (if possible) and whether it is representative
of the whole coast. See `?calc_iphc_full_res` for full details of the output,
...
```{r serABCD}
series_all <- calc_iphc_full_res(sp_set_counts$set_counts)
series_all
```
To plot Series A and B and show their scaled versions and Series AB:
