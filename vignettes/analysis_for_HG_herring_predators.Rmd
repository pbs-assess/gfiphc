...
grep("skate", unique(countData2013$spNameIPHC))

# 2020 (not in GFbio) TODO
```
So, we can use `unident. Skate` as a species name (done above). Load in and
examine that data:
```{r uni}
uni <- readRDS(sp_hyphenate(skate_unident))  # gives no catches at all:
summary(uni)

# But there should be some:
dplyr::filter(countData1995, spNameIPHC == "unident. Skate")
```
