...
HERE - go through this from top, and finally can add unidentified to the species
vector.
...
For at least 1995 and 1996 there are 'Unident. skate', and can see from species figures
below that no skate species were indeed identified for those years. And it looks
like maybe later years also have undentified skate yet big and longnose were
identified.
...
examine that data:
```{r uni}
uni <- readRDS(sp_hyphenate(skate_unident))
summary(uni$set_counts)
dplyr::select(uni$set_counts,
              year) %>%
  table()
```

This commit changes the empty data to not have 2003 with a row of NA's, so that
the above now has no 2003 data.

And check 2013 has no unidentified skate:
```{r details}
dplyr::filter(uni$set_counts, year == 2013) %>% dplyr::select(C_it20) %>% max()
```

This works now:
```{r unident, warning = FALSE, results = FALSE}
for(sp in skate_unident){
  assign(sp_hyphenate(sp, underscore = TRUE, area = TRUE),
         iphc_get_calc_plot_area(sp))
}
```
...
Now want to lump the skates in `skate_sp` together:
...
# How many NA's in certain columns:
res$set_counts %>% dplyr::select(-c("station", "lat", "lon", "usable", "standard")) %>%
  dplyr::summarise_all( ~ sum(is.na(.x))) %>%
    t()
```

Note that species counts get combined at `N_it.1` etc. While each one has NA's,
none of the summed `N_it_sum` or `N_it20_sum` do, because
`get_combined_species()` ignores NA's when making the sums.

TODO: HERE - move these into get_combined_species().
Then adapt series E/F calculation to use the above output - just need to remove
extra columns and rename `N_it_sum` and `N_it20_sum` to `N_it` and `N_it20`.

```{r testcombine, warning = FALSE}
# `get_combined_species()` properly deals with how NA's pass through. Want them
#  to stay NA's if true, but do we ever get N_it_sum = NA  when E_it != NA but
#  usable = "Y"? This will break if we do (TODO could move into function).
...
# But no positive counts after 2000
dplyr::select(unidentified_skate_area$ser_E_and_F$ser_E, year, num_pos20)
dplyr::select(unidentified_skate_area$ser_E_and_F$ser_F, year, num_pos)
```


## Individual non-skate species

Not evaluating for now (since copied from above):
```{r showmult, warning = FALSE, eval=FALSE}
for(sp in sp_vec){
  assign(sp_hyphenate(sp, underscore = TRUE, area = TRUE),
         iphc_get_calc_plot_area(sp))
}
