...
So the problem is that we find no records in GFbio, and in such a case the
extraction code a row of NA's (to return something):
```{r details}
dplyr::filter(uni$set_counts, year == 2003)
```
but the area code then doesn't like that. May as well remove this row from
analyses.
